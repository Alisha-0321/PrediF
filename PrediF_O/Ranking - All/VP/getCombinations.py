...
def heuristic_distance_to_victim(oldRank,order,isPass,victim_index,method):
    noOfMethodsBetween=len(order[:victim_index])-order.index(method)
...
def updateRank(oldRank,order,isPass,victim_index,method,mode):
    if mode=="1":
...
def updateRanks(git,module,victim,order,isPass,lastRanks):
    newRanks={}
    subOrder=order[:order.index(victim)]
    for method in subOrder:
        newRanks[method]=updateRank(lastRanks[method],order,isPass,order.index(victim),method,"1")
    if not are_values_unique(newRanks):
        for method in subOrder:
            newRanks[method]=updateRank(newRanks[method],order,isPass,order.index(victim),method,"2")
        if not are_values_unique(newRanks):
            for method in subOrder:
                newRanks[method]=updateRank(newRanks[method],order,isPass,order.index(victim),method,"3")
    for method in lastRanks.keys():
        if method not in newRanks.keys():
            newRanks[method]=lastRanks[method]
    return newRanks

def getUpdatedRankListPerVictim(git,module,victim,order,lastRanks,vpc_bss_data):
    vpc_data=vpc_bss_data[git][module]
    isPass=getPassOrFail(order,victim,vpc_data)
    rankList=updateRanks(git,module,victim,order,isPass,lastRanks)
    ...
    for i,order in enumerate(orders):
        ...
        for victim in victims:
            if i==0:
                data[victim][str(i-1)]={}
                for method in order:
                    data[victim][str(i-1)][method]=0
                data[victim][str(i-1)]=sort_dict(data[victim][str(i-1)])
            data[victim][str(i)]=getUpdatedRankListPerVictim(git,module,victim,order,data[victim][str(i-1)],vpc_bss_data)
            data[victim][str(i)]=sort_dict(data[victim][str(i)])
    return data
        ...
        for module in data[git].keys():
            if module!="sha":
                for victim in data[git][module].keys():
                    for order in data[git][module][victim].keys():
                        if order!="-1":
                            print("writing file git: "+git+", module: "+module+", victim: "+victim+", i: "+order)
                            location="output/"+git.replace("/","#")+"/"+module.replace("/","#")+"/"+victim.replace("/","#")+"/"
                            mkdir(location)
                            writeFile(location+order+".txt",str(data[git][module][victim][order]))                 
