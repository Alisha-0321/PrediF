...
def heuristic_methods_before_brittle(oldRank,order,isPass,brittle_index):
    noOfMethods=len(order[:brittle_index])
    if isPass:
        return oldRank+(1/noOfMethods)
    else:
        return oldRank-(1/noOfMethods)
    
def heuristic_distance_to_brittle(oldRank,order,isPass,brittle_index,method):
    noOfMethodsBetween=len(order[:brittle_index])-order.index(method)
...
def updateRank(oldRank,order,isPass,brittle_index,method,mode):
    if mode=="1":
        newRank=heuristic_one(isPass,oldRank)
    elif mode=="2":
        newRank=heuristic_distance_to_brittle(oldRank,order,isPass,brittle_index,method)
    elif mode=="3":
        newRank=heuristic_methods_before_brittle(oldRank,order,isPass,brittle_index)    
    return newRank
...
def getPassOrFail(order,brittle,vpc_data):
    subOrder=order[:order.index(brittle)]
    Pass=True
    isPolluted=True
    for statesetter in vpc_data["statesetters"][brittle]:
        if statesetter in subOrder:
            isPolluted=False
            break
    if isPolluted:
        return not Pass
...
def updateRanks(git,module,brittle,order,isPass,lastRanks):
    newRanks={}
    subOrder=order[:order.index(brittle)]
    for method in subOrder:
        newRanks[method]=updateRank(lastRanks[method],order,isPass,order.index(brittle),method,"1")
    if not are_values_unique(newRanks):
        for method in subOrder:
            newRanks[method]=updateRank(newRanks[method],order,isPass,order.index(brittle),method,"2")
        if not are_values_unique(newRanks):
            for method in subOrder:
                newRanks[method]=updateRank(newRanks[method],order,isPass,order.index(brittle),method,"3")    
    for method in lastRanks.keys():
        if method not in newRanks.keys():
            newRanks[method]=lastRanks[method]
    return newRanks

def getUpdatedRankListPerBrittle(git,module,brittle,order,lastRanks,vpc_bss_data):
    vpc_data=vpc_bss_data[git][module]
    isPass=getPassOrFail(order,brittle,vpc_data)
    rankList=updateRanks(git,module,brittle,order,isPass,lastRanks)
    if brittle in rankList.keys():
        del rankList[brittle]
    return rankList
...
def getRankedLists(git,module,orders,vpc_bss_data):
    ...
    for i,order in enumerate(orders):
        ...
        for brittle in brittles:
            if i==0:
                data[brittle][str(i-1)]={}
                for method in order:
                    data[brittle][str(i-1)][method]=0
                data[brittle][str(i-1)]=sort_dict(data[brittle][str(i-1)])
            data[brittle][str(i)]=getUpdatedRankListPerBrittle(git,module,brittle,order,data[brittle][str(i-1)],vpc_bss_data)
            data[brittle][str(i)]=sort_dict(data[brittle][str(i)])
    return data
    ...
    for git in ordersPerProj.keys():
        for module in ordersPerProj[git].keys():
            ...
            if module!="sha":
                ...
                for brittle in rankedListsPerVictim.keys():
                    if git not in data.keys():
                        data[git]={}
                        data[git]["sha"]=sha
                    if module not in data[git].keys():
                        data[git][module]={}
                    data[git][module][brittle]=rankedListsPerVictim[brittle]
...
def printData(data):
    for git in data.keys():
        for module in data[git].keys():
            if module!="sha":
                for brittle in data[git][module].keys():
                    for order in data[git][module][brittle].keys():
                        if order!="-1":
                            print("writing file git: "+git+", module: "+module+", brittle: "+brittle+", i: "+order)
                            location="output/"+git.replace("/","#")+"/"+module.replace("/","#")+"/"+brittle.replace("/","#")+"/"
                            mkdir(location)
                            writeFile(location+order+".txt",str(data[git][module][brittle][order]))                 

def main():
    csv_file="randomOrders.csv"
    print("---reading random orders for projects")
    csv_data=readLargeCsv(csv_file,True)
    ...
    ordersPerProj=procressOrdersPerProj(csv_data)
    data=generateRanks(ordersPerProj,vpc_bss_data)
    printData(data)
