...
from transformers import AdamW, AutoTokenizer, AutoModel, AutoConfig, AutoModelForSequenceClassification
...
# Check the maximum position embeddings (maximum sequence length)
...
#auto_model = AutoModelForSequenceClassification.from_pretrained(model_name)
...
def text_to_tensors(tokens_train, tokens_val, tokens_test):
    ...
    count_tmp +=1
    #if count_tmp==1:
    #    print(tokens_test['attention_mask'][0])
    ...
    print("Input IDs shape (train):", train_seq.shape)
    print("Attention Mask shape (train):", train_mask.shape)
...
def evaluate():
    ...
    for step, batch in enumerate(val_dataloader):
        ...
        sent_id, mask, labels = batch
        total_seq_length = sent_id.size(1)  # Assuming the sequence length is the second dimension

        # Split the sequence into non-overlapping chunks
        '''for start in range(0, total_seq_length, chunk_size):
            end = min(start + chunk_size, total_seq_length)
            chunk_sent_id = sent_id[:, start:end]
            chunk_mask = mask[:, start:end]
            ...
            with torch.no_grad():
                # model predictions
                preds = model(chunk_sent_id, chunk_mask)
...
for train_index, test_index in skf.split(input_data, target_data):
    ...
    '''X_test=np.load(data_name+'/data_splits/X_test_fold'+str(fold_number)+'.npy',allow_pickle=True)
    y_test=np.load(data_name+'/data_splits/y_test_fold'+str(fold_number)+'.npy',allow_pickle=True)

    X_train=np.load(data_name+'/data_splits/X_train_fold'+str(fold_number)+'.npy',allow_pickle=True)
    y_train=np.load(data_name+'/data_splits/y_train_fold'+str(fold_number)+'.npy',allow_pickle=True)

    X_valid=np.load(data_name+'/data_splits/X_valid_fold'+str(fold_number)+'.npy',allow_pickle=True)
    y_valid=np.load(data_name+'/data_splits/y_valid_fold'+str(fold_number)+'.npy',allow_pickle=True)'''
