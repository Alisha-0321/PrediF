...
#input_data = df['victim_code'] # use the 'full_code' column to run Flakify using the full code instead of pre-processed code
...
if data_name == "Victim_Polluter_Pair-Data":
    ...
    columns_to_check = ['victim_code', 'c_or_nc_code', 'polluter_code'] 
...
#nan_val=df[df['c_or_nc_code'].isna()]

#df1 = pd.DataFrame(data,columns=['first_set','second_set'])
#print(nan_val)
...
def tokenize_data(train_text, val_text, test_text):
    tokenized_train_chunks = []
    tokenized_valid_chunks = []
    tokenized_test_chunks = []
    for item in train_text:
        ...
        print(tokens_train)
        print("DONEEE**")
    print('************len======',len(train_text_chunk[0]))
    print('************chunk[0]======',train_text_chunk[0])

    for chunk in train_text_chunk: 
        tokens_train = tokenizer.batch_encode_plus(
            chunk.tolist(),
            max_length=512,
            pad_to_max_length=True,
            truncation=True)
        tokenized_train_chunks.append(tokens_train)
    
    for chunk in val_text_chunk: 
        tokens_val = tokenizer.batch_encode_plus(
            chunk.tolist(),
            max_length=512,
            pad_to_max_length=True,
            truncation=True)
        tokenized_valid_chunks.append(tokens_val)

    for chunk in test_text_chunk: 
        tokens_test = tokenizer.batch_encode_plus(
            chunk.tolist(),
            max_length=512,
            pad_to_max_length=True,
            truncation=True)
        tokenized_test_chunks.append(tokens_test)

    #return tokens_train, tokens_val, tokens_test
    return tokenized_train_chunks, tokenized_valid_chunks, tokenized_test_chunks

# convert vector representation to tensors
def text_to_tensors(tokenized_chunks_train, tokenized_chunks_val, tokenized_chunks_test):
	# Initialize empty lists to store tensors
    #input_ids_train_list = []
    #attention_mask_train_list = []
    #for tokens in tokenized_chunks_train:
        #print('TOKENS')
        #print(tokens.keys())
        #input_ids_train_list.append(torch.tensor(tokens['input_ids']))
        #attention_mask_train_list.append(torch.tensor(tokens['attention_mask']))

    # Concatenate the tensors
    #input_ids_train = torch.cat(input_ids_train_list, dim=0)
    #attention_mask_train = torch.cat(attention_mask_train_list, dim=0) 

    input_ids_train = torch.cat([torch.tensor(tokens['input_ids']) for tokens in tokenized_chunks_train], dim=0)
    attention_mask_train = torch.cat([torch.tensor(tokens['attention_mask']) for tokens in tokenized_chunks_train], dim=0)

    input_ids_val = torch.cat([torch.tensor(tokens['input_ids']) for tokens in tokenized_chunks_val], dim=0)
    attention_mask_val = torch.cat([torch.tensor(tokens['attention_mask']) for tokens in tokenized_chunks_val], dim=0)

    input_ids_test = torch.cat([torch.tensor(tokens['input_ids']) for tokens in tokenized_chunks_test], dim=0)
    attention_mask_test = torch.cat([torch.tensor(tokens['attention_mask']) for tokens in tokenized_chunks_test], dim=0)

    #return train_seq, train_mask, val_seq, val_mask, test_seq, test_mask
    return input_ids_train, attention_mask_train, input_ids_val, attention_mask_val, input_ids_test, attention_mask_test
...
class BERT_Arch(nn.Module):
    ...
    def __init__(self, bert):
        ...
        # dense layer 2 (Output layer), For FlakiCat=5, IDOFT=6, binary-classification=2
...
def give_test_data_in_chunks(x_test): #BERT
    ...
    for g, x_test_chunk in x_test.groupby(np.arange(len(x_test)) // n):
        tokens_test = tokenizer.batch_encode_plus(
            x_test_chunk.tolist(), max_length=510, pad_to_max_length=True, truncation=True)
...
for train_index, test_index in skf.split(input_data, target_data):
    ...
    #X_valid_chunks = [X_valid[i:i+chunk_size] for i in range(0, len(X_valid), chunk_size)]
    #X_test_chunks = [X_test[i:i+chunk_size] for i in range(0, len(X_test), chunk_size)]

    # Tokenize and process chunks sequentially
    #tokenized_chunks_train,tokenized_chunks_val,tokenized_chunks_test  = tokenize_data(X_train_chunks,X_valid_chunks,X_test_chunks)
    ...
    print(type(tokens_train))
    print(type(tokens_val))
    print(type(tokens_test))
